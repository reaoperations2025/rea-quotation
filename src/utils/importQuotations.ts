...
import quotationsData from "@/data/quotations-2025-full.json";
...
export async function importQuotationsFromJSON() {
  try {
    // Get current user
    ...
    const quotations = (quotationsData as any).quotations as QuotationData[];
    
    console.log(`Starting import of ${quotations.length} quotations...`);
...
function formatDate(dateStr: string): string {
  ...
  try {
    ...
    // Format as DD-Mon-YY (to match existing format)
